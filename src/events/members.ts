    ...
    async memberAdd([member]: ArgsOf<"guildMemberAdd">, client: Client) {
        ...
        if (member.user.bot) {
            ...
            if (
                ...
                try {
                    await member.send(
                        "W celu ochrony przed raidami, botami itp. konta utworzone mniej niż tydzień temu są usuwane z serwera po dołączeniu. Przed ponownym dołączeniem upewnij się, że twoje konto ma co najmniej tydzień."
                    );
                } catch (_) {}
